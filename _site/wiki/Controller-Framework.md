...
DO_SEND_ICC_MSG       | This CBT is generated when a CM has to send a message to another node via Inter-Controller Communication (ICC). `cbt.data` is a dict with keys: `src_uid`, `dst_uid`, `icc_type`, and `msg`.
